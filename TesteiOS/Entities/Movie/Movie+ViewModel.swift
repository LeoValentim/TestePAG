...
extension Movie.Model {
    struct ViewModel {
        ...
        init(from model: Movie) {
            id = model.id
            title = model.title ?? ""
            releaseDate = model.releaseDate?.toDate(withFormat: "yyyy-MM-dd") ?? Date(timeIntervalSince1970: 0)
        ...
        init(from localModel: MovieLocal) {
            ...
            releaseDate = localModel.releaseDate ?? Date(timeIntervalSince1970: 0)
