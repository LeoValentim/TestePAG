...
extension MovieDetailsViewController {
    func setupViews() {
        ...
        dateLabel.text = model?.releaseDate.toString(withFormat: "d 'de' MMMM 'de' yyyy")
        categoryLabel.text = model?.genres.filter({ $0.name != nil }).map({ $0.name! }).joined(separator: ", ")
        descriptionLabel.text = model?.overview
        
        let icon: UIImage?
        if model?.isFavorite == true {
            icon = UIImage(named: "favorite_gray_icon")?.withRenderingMode(.alwaysTemplate)
        } else {
            icon = UIImage(named: "favorite_empty_icon")?.withRenderingMode(.alwaysTemplate)
        }
